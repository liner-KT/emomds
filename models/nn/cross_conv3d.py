...
class CrossConv3d(nn.Conv3d):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        xs = E.repeat(x, "B Sx Cx D H W -> B Sx Sy Cx D H W", Sy=Sy)
        ys = E.repeat(y, "B Sy Cy D H W -> B Sx Sy Cy D H W", Sx=Sx)

        xy = torch.cat([xs, ys], dim=3,)

        batched_xy = E.rearrange(xy, "B Sx Sy C2 D H W -> (B Sx Sy) C2 D H W")
        batched_output = super().forward(batched_xy)

        output = E.rearrange(
            batched_output, "(B Sx Sy) Co D H W -> B Sx Sy Co D H W", B=B, Sx=Sx, Sy=Sy
        )
        return output
